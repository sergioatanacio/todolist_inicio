...
import {
  type DeleteSegmentoTiempoCommand,
  validateDeleteSegmentoTiempoCommand,
} from '../../commands/disponibilidad/DeleteSegmentoTiempoCommand'

export class DeleteSegmentoTiempoUseCase {
  constructor(
  ...
  async execute(command: DeleteSegmentoTiempoCommand) {
    const input = validateDeleteSegmentoTiempoCommand(command)
    return this.unitOfWork.runInTransaction(async () => {
      ...
      const nextDisponibilidad = disponibilidad.removeSegment(input.segmentId)
